<template>
		<div class="nx-menu-wrapper" v-context-menu="contextMenus.empty" @contextmenu.stop="handleSessionTreeContainerClick"
				...
				<el-scrollbar style="height: calc(100% - 32px)">
					...
					<span class="custom-tree-node" @dblclick.stop="handleHostOpen(data)">
						...
						<span class="session-extend" style='--n-hover-bg-color: #00000018'>
							<el-tooltip v-if="type === 'node' && protocol === 'ssh'" effect="dark"
							            :content="$t('home.sessions-context-menu.sftp')" placement="top-start">
						...
						<el-empty v-show="menuTree.length === 0"
...
onMounted(() => {
		...
		// 订阅主机搜索事件
		subscript("nx-config-sync", async (keywords) => {
				try {
